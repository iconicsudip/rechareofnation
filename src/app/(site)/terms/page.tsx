...
export default function TermsConditionsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] container py-20 md:py-24 flex flex-col gap-10 max-w-4xl text-left">
      <div>
        <span className="text-sm font-semibold uppercase tracking-widest text-pink-500">Legal Documentation</span>
        <h1 className="text-4xl font-black mt-1 font-primary text-slate-900">{content?.heading}</h1>
        <p className="text-gray-500 text-xs mt-2">Last Updated: {content?.lastUpdated}</p>
      </div>

      <div className="bg-white border border-slate-200/90 rounded-[28px] shadow-sm p-8 md:p-10 flex flex-col gap-6 text-sm text-slate-600 leading-relaxed font-secondary">
        <p>
          {content?.introText}
        </p>
        ...
        {(content?.sections ?? []).map((section, idx) => (
          <div key={idx} className="flex flex-col gap-6">
            <h3 className="text-lg font-bold text-slate-900 font-primary mt-4">{section.title}</h3>
